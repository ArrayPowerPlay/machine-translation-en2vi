...
async def login(user: schemas.UserLogin, db: Session = Depends(database.get_db)):
    """Hỗ trợ đăng nhập cho người dùng"""
    db_user = db.query(db_models.User).filter(db_models.User.username == user.username).first()
...
async def translate_text(
    ...
    try:
        ...
        # Lưu lịch sử dịch nếu người dùng đăng nhập
        if current_user:
            # Kiểm tra và loại bỏ các bản dịch trùng lặp
            # Lấy bản dịch mới nhất
...
async def delete_history_item(
    ...
    """Xóa một bản ghi trong lịch sử dịch"""
    item = db.query(db_models.TranslationHistory) \
        .filter(db_models.TranslationHistory.id == history_id, db_models.TranslationHistory.user_id == current_user.id).first()
    if not item:
        raise HTTPException(status_code=404, detail="Item not found")
    
    # Logic: Xóa bản dịch ở history -> Xóa bản dịch đó trong bảng đã lưu 
...
async def get_saved_translations(
    ...
    if search:
        query = query.filter(db_models.SavedTranslation.original_text.contains(search))
...
async def delete_saved_translation(
    ...
    item = db.query(db_models.SavedTranslation).filter(db_models.SavedTranslation.user_id == current_user.id, db_models.SavedTranslation.id == id).firt()
    if not item:
        raise HTTPException(status_code=404, detail="Item not found")
...
if __name__ == "__main__":
    import uvicorn
    # Backend lắng nghe từ tất cả các IP
